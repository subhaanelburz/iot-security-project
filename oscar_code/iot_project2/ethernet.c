    ...
    if (kbhitUart0())
    {
        ...
        else
        {
            ...
            if (strcmp(token, "help") == 0)
            {
                ...
                putsUart0("  nfc\n");
                putsUart0("  pir on|off\n");
                putsUart0("  sensors (status)\n");
                putsUart0("  sonic on|off\n");
                putsUart0("  stop\n");
                putsUart0("  ip\n");
            ...
            {
                if (!isPirEnabled())
                    putsUart0("PIR is off\n");
                else if (pirMotionDetected())
                {
                    if (!isSonicEnabled())
                        putsUart0("PIR active, sonic off\n");
                    else if (measureUltrasonicDistanceCm(&distanceCm))
                    {
                        snprintf(str, sizeof(str), "PIR active, distance %u cm\n", distanceCm);
                        putsUart0(str);
                    }
                    else
                        putsUart0("PIR active, ultrasonic timeout\n");
                }
                else
                    putsUart0("PIR idle\n");
            }
            if (strcmp(token, "pir") == 0)
            {
                ...
                if ((token != NULL) && (strcmp(token, "on") == 0))
                {
                    setPirEnabled(true);
                    putsUart0("PIR on\n");
                    if (isSonicEnabled() && pirMotionDetected())
                        startSensorSampling();
                }
            ...
            if (strcmp(token, "sonic") == 0)
            {
                ...
                if ((token != NULL) && (strcmp(token, "on") == 0))
                {
                    setSonicEnabled(true);
                    putsUart0("Sonic on\n");
                    if (isPirEnabled() && pirMotionDetected())
                        startSensorSampling();
                }
                else if ((token != NULL) && (strcmp(token, "off") == 0))
                {
                    setSonicEnabled(false);
                    putsUart0("Sonic off\n");
                }
                else
                    putsUart0("Use: sonic on|off\n");
            }
            if (strcmp(token, "stop") == 0)
            {
                stopPirAndSonic();
                putsUart0("PIR and sonic off\n");
            }
        }
    }
}
...
int main(void)
{
    ...
    uint16_t sensorPollDivider;
    uint16_t sensorDistanceCm;
    char nfcMqttTopic[32];
    char nfcMqttPayload[96];
    char sensorMqttTopic[32];
    char sensorMqttPayload[64];
    char sensorStatus[48];
    etherHeader *data = (etherHeader*) buffer;
    socket s;
    bool nfcPresent;
    bool nfcPublishPending;
    bool pirDetected;
    bool lastPirDetected;
    bool sensorDistanceValid;
    bool sensorPublishPending;
    ...
    putsUart0("Starting PIR + ultrasonic sensors\n");
    ...
    while (true)
    {
        ...
        // Publish pending sensor data once MQTT is connected
        if (nfcPublishPending || sensorPublishPending)
        {
            if (!isMqttConnected())
                connectMqtt();
            else
            {
                ...
                {
                    publishMqtt(sensorMqttTopic, sensorMqttPayload);
                    putsUart0("MQTT publish ");
                    putsUart0(sensorMqttPayload);
                    putsUart0("\n");
                    sensorPublishPending = false;
                }
            }
        ...
        // PIR and ultrasonic polling
        if (!kbhitUart0() && (++sensorPollDivider >= 20))
        {
            sensorPollDivider = 0;
            if (!isPirEnabled())
            {
                ...
                {
                    stopSensorSampling();
                    queueSensorMqttPublish(false, false, 0, sensorMqttTopic, sizeof(sensorMqttTopic), sensorMqttPayload, sizeof(sensorMqttPayload));
            ...
            {
                pirDetected = pirMotionDetected();

                if (pirDetected && !lastPirDetected)
                {
                    if (isSonicEnabled())
                        startSensorSampling();
                    else
                        clearSensorSampleRequest();
                    putsUart0("PIR triggered\n");
                }
                else if (!pirDetected && lastPirDetected)
                {
                    stopSensorSampling();
                    queueSensorMqttPublish(false, false, 0, sensorMqttTopic, sizeof(sensorMqttTopic), sensorMqttPayload, sizeof(sensorMqttPayload));
                    sensorPublishPending = true;
                    putsUart0("PIR cleared\n");
                }

                if (pirDetected && isSensorSampleRequested() && isSonicEnabled())
                {
                    clearSensorSampleRequest();
                    sensorDistanceValid = measureUltrasonicDistanceCm(&sensorDistanceCm);
                    if (sensorDistanceValid)
                    {
                        snprintf(sensorStatus, sizeof(sensorStatus), "Distance %u cm\n", sensorDistanceCm);
                        putsUart0(sensorStatus);
                    }
                    else
                        putsUart0("Ultrasonic timeout\n");
                    queueSensorMqttPublish(true, sensorDistanceValid, sensorDistanceCm,
                                           sensorMqttTopic, sizeof(sensorMqttTopic), sensorMqttPayload, sizeof(sensorMqttPayload));
                    sensorPublishPending = true;
                }
